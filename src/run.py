...
parser.add_argument("--ONR", type=str2bool, default=True, help="whether to use ONR in training")
parser.add_argument("--use_sequential", type=str2bool, default=False)
parser.add_argument("--use_shared_encoder", type=str2bool, default=True)
...
def main(limit=1e9):
    ...
    if args.ONR:
        logger.info("ONR is ON → skip training dataset cache (build headers only and resample per step)")
        ...
        if os.path.exists(test_cache):
            ...
            test_dataset = MultiDataset(
                ...
                precomputed_features=loaded["features"], is_train=False, train_examples=train_examples, ONR=False
            )
            logger.info(f"Loaded cached test dataset from {test_cache}")
        else:
            test_dataset = MultiDataset(
                vocabulary, test_examples, args.max_arity, args.max_seq_len, args.max_per_type, args.max_neighbors_num,
                is_train=False, train_examples=train_examples, ONR=False
            )
        ...
        if os.path.exists(train_cache):
            ...
            train_dataset = MultiDataset(
                vocabulary, train_examples, args.max_arity, args.max_seq_len, args.max_per_type, args.max_neighbors_num,
                precomputed_features=loaded["features"], is_train=True, ONR=False
            )
            logger.info(f"Loaded cached train dataset from {train_cache}")
        else:
            train_dataset = MultiDataset(
                vocabulary, train_examples, args.max_arity, args.max_seq_len, args.max_per_type, args.max_neighbors_num,
                is_train=True, ONR=False
            )
    ...
    if len(args.device) > 1:
        model = torch.nn.DataParallel(GLoRE(len(vocabulary.vocab), vocabulary.num_relations, args.num_entities, args.max_arity, args.sync_layers, args.fusion_layers,
                                            ...
                                            args.num_bin, args.rel_layers, args.use_relgraph, args.use_global, args.use_sequential, args.use_shared_encoder, args.use_gating, rel_triplets), device_ids=devices)
        model.to(device)
    else:
        model = GLoRE(len(vocabulary.vocab), vocabulary.num_relations, args.num_entities, args.max_arity, args.sync_layers, args.fusion_layers,
                      args.trans_layers, args.hidden_dim, args.local_heads, args.global_dropout, args.local_dropout, args.global_activation, args.decoder_activation,
                      args.num_head, args.remove_mask, args.dim_rel, args.hid_dim_ratio_rel,
                      args.num_bin, args.rel_layers, args.use_relgraph, args.use_global, args.use_sequential, args.use_shared_encoder, args.use_gating, rel_triplets).to(device)
