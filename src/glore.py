...
class GLoRE(nn.Module):
    def __init__(
        ...
        if not self.use_shared_encoder:
            self.pair_projector_local = PairProjector(hidden_dim, sync_layers, num_entity_roles=3)
        ...
        if self.use_shared_encoder:
            ...
            self.local_module = LocalPairLayer(
                ...
                decoder_activation, remove_mask,
                bias, trans_layers, times=times,
                pair_projector=self.pair_projector_local, pair_layers=sync_layers
            )
    ...
    def forward(
        ...
        if self.use_sequential:
            h_emb = None
            x_global, emb, __ = self.global_module(emb, 
                                                    input_ids,
                                                    neighbor_graph["fact_rel_ids"], 
                                                    neighbor_graph["fact_ent_ids"], 
                                                    neighbor_graph["fact_entity_roles"], 
                                                    ...
                                                    neighbor_graph["fact_pair_mask"], 
                                                    h_emb=h_emb)
            fused_seq = torch.nn.functional.embedding(input_ids, emb)
            fused_seq = self.input_dropout(self.input_norm(fused_seq))
            for layer_idx in range(self.sync_layers):
                fused_seq = self.local_module(fused_seq, input_mask, pair_layer_idx=layer_idx)
            ...
            for _ in range(self.fusion_layers):
                ...
                if self.use_gating:
                    ...
                    fused_seq = self.joint_layer.layer_norm(0.5 * x_local + 0.5 * x_global + fused_seq)
